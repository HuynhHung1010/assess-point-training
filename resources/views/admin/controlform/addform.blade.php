...
@section('content')
    <main>
        <h1 class="title">Quản lý biểu mẫu</h1>
            ...
            @endif
            <form action="{{route('admin.postaddform')}}" method="POST" class="row mb-3" >
                @csrf
                {{-- <div class="border border-dark"> --}}
                    <div class="col-md-4">
                    <label for="inputEmail4" class="form-label">Mã tiêu chí</label>
                    <input type="text" class="form-control" id="inputEmail4" name="matc" placeholder="Mã tiêu chí..." value="{{old('matc')}}">
                    @error('matc')
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="inputAddress" name="tentc" placeholder="Tên tiêu chí..." value="{{old('tentc')}}">
                        @error('tentc')
                            <span style="color: red;">{{$message}}</span>
                        @enderror
                    </div>
                    ...
                    <input type="text" class="form-control" id="inputPassword4" name="diemmax" placeholder="Điểm tối đa..." value="{{old('diemmax')}}">
                    @error('diemmax')
                        <span style="color: red;">{{$message}}</span>
                    @enderror
                    </div>
                ...
                {{-- </div> --}}
                    <div class="col-12 p-3">
                        <button type="submit" class="btn btn-success">Thêm tiêu chí  <i class="fa-solid fa-check"></i></button>
